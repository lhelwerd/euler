...
def _special_subset_sum(comb: Collection[int], comb2: Collection[int]) -> bool:
    if sum(comb) == sum(comb2):
        return False
    return not (len(comb) > len(comb2) and sum(comb) < sum(comb2))
        ...
        for comb in itertools.combinations(candidate, length):
            ...
            for rest_length in range(1, len(rest) + 1):
                for comb2 in itertools.combinations(rest, rest_length):
                    if not _special_subset_sum(comb, comb2):
                        return False
