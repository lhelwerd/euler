...
The modules are written for Python 3.10+ and for PyPy 3.10+. Support for 
versions before 3.8 is dropped in version 0.0.2, support for 3.8 is dropped in 
version 0.0.3 and support for 3.9 is dropped in version 0.0.4. The modules 
support running on [PyPy](https://www.pypy.org/) (currently those based on 
Python 3.10) as well. Detailed information on changes for each version is found 
...
installation of the module is accomplished with `uv add`, `pip install` or 
`poetry add` followed by either the release zip/tarball or the current 
directory. `make install` installs from the current directory. We recommend 
using virtual environments to keep your dependencies separate from global 
installation.
...
a `pyproject.toml`, `requirements.txt` or similar. Other syntax exists to mark 
dependency for a specific release, see [requirements file 
